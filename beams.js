...
export class BeamsBackground {
  constructor(options = {}) {
    this.beamWidth = options.beamWidth || 2;
    this.beamHeight = options.beamHeight || 15;
    this.beamNumber = options.beamNumber || 12;
    this.lightColor = options.lightColor || '#1fb1ef';
    this.speed = options.speed || 2;
    this.noiseIntensity = options.noiseIntensity || 1.75;
    ...
    Object.assign(this.container.style, {
      ...
      width: '100vw',
      height: '100vh',
      zIndex: -2, // Behind existing backgrounds
      pointerEvents: 'none',
      overflow: 'hidden'
    });
    ...
    this.handleResize(); // Force initial resize to calculate dynamic scale
  ...
  buildBeams() {
    this.beamMaterial = extendMaterial({
      header: `
varying vec3 vEye;
...
vec3 getNormal(vec3 pos) {
  ...
  gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
      ...
      uniforms: {
        ...
        envMapIntensity: 10,
        uNoiseIntensity: this.noiseIntensity,
        uScale: this.scale
      }
    });

    const geometry = createStackedPlanesBufferGeometry(this.beamNumber, this.beamWidth, this.beamHeight, 0, 100);
    this.mesh = new THREE.Mesh(geometry, this.beamMaterial);
  ...
  addLights() {
    ...
    // Setup shadow props if shadows were enabled, but just positioning here is enough
  ...
  handleResize() {
    ...
    // Dynamically scale the beams group so it ALWAYS fills the screen, 
    // especially on ultrawide monitors or zoomed out views.
    if (this.group) {
        const aspect = window.innerWidth / window.innerHeight;
        // Base scale of 2 ensures it covers normal screens, aspect * 1.5 accounts for ultrawide
        const scale = Math.max(aspect * 1.5, 2.5);
  ...
  animate() {
    ...
    if (this.beamMaterial && this.beamMaterial.uniforms.time) {
        this.beamMaterial.uniforms.time.value += 0.1 * delta;
    }
